					...
					<div class="content">
						...
						<ul class="cl-vnavigation">
							...
							<li><a href="#"><i class="fa fa-desktop"></i><span>新闻管理</span></a>
								<ul class="sub-menu">
									<li class="active"><a href="findPageAllNew.do">查看新闻</a></li>
									...
									<table style="clear: both"
										...
										<tbody>
											...
											<c:forEach var="u" items="${news}">
												<tr>
													<td><a href="getAdminNew.do?newID=${u.newID}">${u.title}</a></td>
													<td><a>${u.time}</a></td>
													<td><a>${u.pusher}</a></td>
													<td><a onclick="ajaxTest(${u.newID},${page.pageNo});"><button
											...
											<div class="compose">
												<c:choose>
													<c:when test="${page.pageNo!=1}">
														<a class="btn btn-flat btn-primary"
															href="findPageAllNew.do?pageNo=${page.previousPageNo }">上一页</a>
													</c:when>
													<c:otherwise>
												...
												<c:choose>
													<c:when test="${page.pageNo != page.totalPages}">
														<a class="btn btn-flat btn-primary"
															href="findPageAllNew.do?pageNo=${page.nextPageNo }">下一页</a>
													</c:when>
													<c:otherwise>
...
#color-switcher .color.gray {
	...
	<script type="text/javascript">
		...
		function ajaxTest(newID, pageNo) {
			$.ajax({
				data : {
					'newID' : newID
				},
				...
				success : function(data) {
					alert(data.msg);
					location.href = "findPageAllNew.do?pageNo=" + pageNo;
				}
			});
		}
	</script>
